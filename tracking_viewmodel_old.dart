...
class TrackingViewModel extends ChangeNotifier {
  ...
  /// Verifica si el usuario est├í dentro del cuadrante. Retorna la posici├│n si s├¡.
  Future<Position?> verificarGeofencing({
    ...
    try {
      ...
      if (pos == null) {
        _setError(
            'No se pudo obtener tu ubicaci├│n. Activa el GPS e intenta de nuevo.');
        return null;
      }
      _posicionActual = pos;
      ...
      if (!dentroDeCuadrante) {
        _setError(
            'Debes estar dentro del cuadrante asignado para iniciar la b├║squeda.');
        notifyListeners();
        return null;
      }
      notifyListeners();
  ...
  Future<bool> iniciarBusqueda({
    ...
    try {
      ...
      if (!ok) {
        _setError(
            'No se pudo iniciar el tracking. Revisa los permisos de ubicaci├│n.');
        return false;
      }
      ...
      if (!kIsWeb && Platform.isAndroid) {
        ...
        await TrackingForegroundService().start(
          titulo: 'GPS activo ÔÇö grabando recorrido',
          reporteId: reporteId,
          usuarioId: usuarioId,
        );
      }
  ...
  Future<void> pausarBusqueda() async {
    ...
    if (!kIsWeb && Platform.isAndroid) {
      await TrackingForegroundService()
          .updateText('GPS pausado ÔÇö b├║squeda en espera');
    }
    notifyListeners();
  }
  ...
  Future<void> reanudarBusqueda() async {
    ...
    if (!kIsWeb && Platform.isAndroid) {
      await TrackingForegroundService()
          .updateText('GPS activo ÔÇö grabando recorrido');
    }
    notifyListeners();
  }
  ...
  void dispose() {
    _refreshTimer?.cancel();
    _removeServiceCallback?.call();
    // No llamamos _trackingService.reset() aqu├¡ porque TrackingViewModel vive
    // para toda la sesi├│n de la app (global provider). El GPS sigue activo en
